...
import { AUTH_SESSION_KEY, AUTH_USER_KEY, DEFAULT_AUTH_BASE_PATH } from '../../src/constants.ts';
...
describe('Better Auth end-to-end flow', () => {
    ...
    test('get-session returns the authenticated user after sign-up', async () => {
        await withAuthApp(async ({ harness }) => {
            ...
            const signUp = await harness.post(`${DEFAULT_AUTH_BASE_PATH}/sign-up/email`, {
                email,
                password: 'password123',
            ...
            const sessionResponse = await harness.get(`${DEFAULT_AUTH_BASE_PATH}/get-session`, {
