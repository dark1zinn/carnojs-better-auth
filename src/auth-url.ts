 ...
 */
export function buildAuthClientBaseURL(appBaseURL: string, basePath: string): string {
...
export function warnOnBaseUrlBasePathMismatch(options: {
    ...
    if (urlPath !== normalizedBasePath) {
        const clientBaseURL = buildAuthClientBaseURL(new URL(baseURL).origin, normalizedBasePath);
